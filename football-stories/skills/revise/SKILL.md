...
| Scripts | `${CLAUDE_PLUGIN_ROOT}/scripts/{tts.py,align.py,validate.py,cards.py,compose.py,finish.py,check.py}` with `PY=<shorts dir>/.venv/bin/python` |
...
2. **Translate** each item into one line: `#k "<their words>" → <field> : <change> → <steps to re-run>`.
   ...
   |---|---|---|---|
   | Card text / layout / colour | `scenes[i].props` | cards (changed cards only) → compose → finish | free |
...
- Channel rules still apply (20–30 s, hook first, clean final card); if the user asks for something that breaks one, do it and note the rule in one line.
