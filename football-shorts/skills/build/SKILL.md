...
3. **Ingest sources, all of them in one pass.** For each URL: `fetch.py <edit_dir> --info <url>` to show what it is, then `fetch.py <edit_dir> --accept-terms --name <key> --rights <class> [--section a-b] <url>`. Use `--section` when the brief gives an approximate time in a long broadcast (a 200 s window around it is plenty). For local files, reference them by absolute path in the spec `sources` map or copy them into `src/`. `sources.json` is the record of origin and rights per clip.
